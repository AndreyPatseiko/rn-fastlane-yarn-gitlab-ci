...
ENV PATH ${PATH}:${ANDROID_HOME}/tools:${ANDROID_HOME}/tools/bin:${ANDROID_HOME}/platform-tools:$GRADLE_HOME
...
RUN cd /usr/local \
	&& wget $GRADLE_URL \
	&& unzip gradle-6.4.1-all.zip \
	&& rm gradle-6.4.1-all.zip \
	&& ln -s /usr/local/gradle-6.4.1/bin/gradle /usr/bin/gradle
...
RUN apt-get install make -y \
	...
	&& apt-get install libxml2-dev libsqlite3-dev zlib1g-dev liblzma-dev -y

RUN gem install fastlane -v 2.129.0 \
	&& gem install fastlane-plugin-appicon \
	&& gem install fastlane-plugin-android_change_string_app_name \
	&& gem install fastlane-plugin-humanable_build_number \
